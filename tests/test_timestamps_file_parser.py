...
def test_from_timestamps_file_empty_string():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps at line 0 is invalid. Here is the line: ""'
    )
...
def test_from_timestamps_file_v1_only_fps_string_int():
    ...
    timestamps, last_frame_time, fpms = TimestampsFileParser.parse_file(
        StringIO(timestamps_str), RoundingMethod.ROUND
    )
...
def test_from_timestamps_file_v1_string_floor():
    ...
    expected_last_frame_time = Fraction(
        1700, 3
    )  # (5 * 1/30 * 1000 + 6 * 1/15 * 1000) = 1700 / 3
    expected_fpms = Fraction(30, 1000)

    timestamps, last_frame_time, fpms = TimestampsFileParser.parse_file(
        StringIO(timestamps_str), RoundingMethod.FLOOR
    )
...
def test_from_timestamps_file_v1_out_of_order():
    ...
    expected_last_frame_time = Fraction(
        1840, 3
    )  # 12 * 1/25 * 1000 + 4 * 1/30 * 1000 = 1840 / 3
    expected_fpms = Fraction(30, 1000)
...
def test_from_timestamps_file_v1_fps_bad_separator():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "5;10;15"'
    )
...
def test_from_timestamps_file_v1_fps_invalid_start_frame():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "5.5,10,15"'
    )
...
def test_from_timestamps_file_v1_fps_invalid_end_frame():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "5,10.5,15"'
    )
...
def test_from_timestamps_file_v1_fps_invalid_range_fps():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "5,10,15a"'
    )
...
def test_from_timestamps_file_v1_end_less_than_start():
    ...
    with pytest.raises(ValueError) as exc_info:
        TimestampsFileParser.parse_file(StringIO(timestamps_str), RoundingMethod.ROUND)
    assert (
        str(exc_info.value) == "End frame must be greater than or equal to start frame."
    )
...
def test_from_timestamps_file_v1_range_missing_info():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "0,1"'
    )
...
def test_from_timestamps_file_v1_range_too_many_info():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "0,1,1,1"'
    )
...
def test_from_timestamps_file_v2_empty_line():
    ...
    assert (
        str(exc_info.value)
        == 'The timestamps file contain a invalid line. Here is it: "\n"'
    )
...
def test_from_timestamps_file_invalid_format():
    ...
    assert (
        str(exc_info.value)
        == "The file uses version 5 for its timestamps, but this format is currently not compatible with PyonFX."
    )

...
def test_from_fps_invalid_fps():
    ...
    assert str(exc_info.value) == "Parameter ``fps`` must be between 0 and 1000 (0 not included)."
...
@pytest.mark.parametrize("fps, expected_output", [
    (
        30,
        Fraction(30, 1000)
    ),
    ...
    (
        24000/1001,
        Fraction(24000, 1001) / Fraction(1000)
    ),
    (
        Fraction(24000, 1001),
        Fraction(24000, 1001) / Fraction(1000)
...
def test_from_timestamps_file_floor():
    ...
    "50.5\n" \
    "100.4\n"
    timestamp = Timestamps.from_timestamps_file(timestamps_str, rounding_method=RoundingMethod.FLOOR)
...
def test_from_video_file_mkv():
    ...
    expected_timestamps = [int(frame * 1/Fraction(24000, 1001) * 1000 + 0.5) for frame in range(500)]
    expected_last_frame_time = int(499 * 1/Fraction(24000, 1001) * 1000 + 0.5) # Important to note, like said in the timestamps.py doc, mkv round timestamps to ms
...
def test_from_video_file_mkv_where_timestamps_arent_in_ms():
    ...
    with pytest.warns(UserWarning, match="Your mkv file isn't perfectly rounded to ms. In this situation, you may prefer to use RoundingMethod.floor then RoundingMethod.ROUND."):
...
def test_from_video_file_invalid_index():
    ...
    assert str(exc_info.value) == f"The index {invalid_index} is not in the file {video_file_path}."
...
def test_from_video_file_index_is_not_a_video():
    ...
    with pytest.raises(ValueError) as exc_info:
        Timestamps.from_video_file(video_file_path, invalid_index)
    assert str(exc_info.value) == f'The index {invalid_index} is not a video stream. It is an "audio" stream.'
...
def test_normalize():
    ...
    assert Timestamps.normalize(positive_timestamps_to_be_normalize, positive_timestamps_to_be_normalize[-1]) == (timestamps_expected_results, last_frame_time_expected_results)
    assert Timestamps.normalize(negative_timestamps_to_be_normalize, negative_timestamps_to_be_normalize[-1]) == (timestamps_expected_results, last_frame_time_expected_results)
...
def test_timestamps_init():
    ...
    assert str(exc_info.value) == "If you specify a value for ``timestamps``, you must specify a value for ``last_frame_time``"
    ...
    assert str(exc_info.value) == "If you don't specify a value for ``timestamps``, you must specify a value for ``fpms``"
    ...
    assert str(exc_info.value) == "If you specify a value for ``fpms``, you cannot specify a value for ``last_frame_time``"

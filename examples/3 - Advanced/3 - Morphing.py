...
def romaji(line: Line, l: Line) -> None:
    ...
    for syl in Utils.all_non_empty(line.syls):
        ...
        if morph_end > bounce_end:
            FU_morph = FrameUtility(int(bounce_end), int(morph_end), io.input_timestamps)
            for s, e, i, n in FU_morph:
                ...
                l.text = (
                    "{\\an7\\pos(%.3f,%.3f)\\p1\\1c%s\\bord2\\3c&H000000&}%s"
                    % (syl.left, syl.top, curr_colour, morphed_shape)
                )
                io.write_line(l)
        ...
        FU_morph_back = FrameUtility(int(leadout_start), int(morph_back_end), io.input_timestamps)
        ...
        if leadout_end > morph_back_end:
            FU_leadout = FrameUtility(int(morph_back_end), int(leadout_end), io.input_timestamps)
...
def sub(line: Line, l: Line, prev_line=None, next_line=None) -> None:
    ...
    l.text = "{\\an5\\pos(%.3f,%.3f)\\fad(%d,%d)}%s" % (line.center, line.middle, int(line.leadin) if prev_line is None else 0, int(line.leadout) if next_line is None else 0, line.text)
    io.write_line(l)
...
romaji_lines = [line for line in Utils.all_non_empty(lines) if line.styleref.alignment >= 7]
sub_lines = [line for line in Utils.all_non_empty(lines) if line.styleref.alignment <= 3]

...
class Shape:
    ...
    def apply_texture(
        ...
        The mapping mode can be one of the following:
            - "stretch": Scale the texture to exactly cover the shape's bounding box.
        ...
        # Compute the bounding box of the shape's pixel mask
        min_x = min(p.x for p in mask_pixels)
        max_x = max(p.x for p in mask_pixels)
        min_y = min(p.y for p in mask_pixels)
        max_y = max(p.y for p in mask_pixels)
        ...
        tex_width = max(p.x for p in texture_pixels) + 1
        tex_height = max(p.y for p in texture_pixels) + 1

        # Build a lookup dictionary for texture pixels
        tex_dict = {(p.x, p.y): p for p in texture_pixels}

        textured_pixels = []
        for mpx in mask_pixels:
            # Map mask pixel to texture coordinates based on mode
            if mode == "stretch":
                u = (mpx.x - min_x) / bb_width
                v = (mpx.y - min_y) / bb_height
                tex_x = int(u * (tex_width - 1))
                tex_y = int(v * (tex_height - 1))
            elif mode == "repeat":
                tex_x = (mpx.x - min_x) % tex_width
            ...
            if tp is None:
                if output_rgba:
                    tp = Pixel(x=tex_x, y=tex_y, color=(255, 255, 255), alpha=255)
                else:
